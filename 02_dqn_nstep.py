...
tgt = reward + (1. - done) * params['gamma'] ** nSteps * max_tgt_q
...
with tf.Session() as sess:
    ...
    summ_name = os.path.join(params['run_name'], f'-{nSteps}-steps-ptan')
    ...
    # Experience source
    exp_source = rl.experience_ptan.ExperienceSourceFirstLast(env, agent,
                    gamma=params['gamma']**nSteps, steps_count=nSteps)
    # Memory buffer
    ...
    with common.RewardTracker(writer) as reward_tracker:
        while frame_idx < total_frames:
            ...
            if frame_idx % 62500 == 0:
            	saver.save(sess, '../Models/'+summ_name+'/'+summ_name, global_step=global_step)
        # Final save
        saver.save(sess, '../Models/'+summ_name+'/'+summ_name, global_step=global_step)
